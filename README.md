...
--tags' for git users).  The tags generated by the `buildrelease` task
in the buildrelease plugin are automatically pushed to remote origins.
...
Note that a variant of this logic is what is behind the	`buildrelease`
target in the buildrelease plugin.
